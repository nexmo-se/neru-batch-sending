...
initializePassport(
  ...
  async (profile) => {
    const globalState = neru.getGlobalState();
    const customer = await globalState.hset("users", {
...
async function processAllFiles(files, assets, scheduler) {
  ...
  for (const filename of files) {
    ...
    if (secondsTillEndOfDay > secondsNeededToSend && utils.timeNow() >= 7) {
      try {
        ...
        try {
          interval = setInterval(() => {
            axios.get(
              `http://${process.env.INSTANCE_SERVICE_NAME}.neru/keepalive`
            );
          }, 1000);
          // const schedulers = await scheduler.listAll().execute();
        ...
        console.log(
          `There are ${secondsTillEndOfDay} sec left and I need ${secondsNeededToSend}`
        );
        const startProcessingDate = new Date().toISOString();
        console.log("file name: " + filename);
        const sendingResults = await smsService.sendAllMessages(
          records,
          filename
        );
        const endProcessingDate = new Date().toISOString();
        const failedResults = sendingResults.filter(
        ...
        const failedPath = filename
          .split("/")[2]
          .replace(".csv", "-failed-output.csv");
        if (failedResults.length > 0) {
          await utils.writeResults(
            failedResults,
            failedPath,
            constants.failedResultsHeader
          );
        ...
        const fileMoved = await utils.moveFile(
          assets,
          processedPath,
          "processed/",
          records,
      ...
      secondsNeededToSend > secondsTillEndOfDay
    ) {
      try {
        console.log(
        ...
        if (failedResults.length > 0) {
          const failedPath = filename
            .split("/")[2]
            .replace(".csv", "-failed-1-output.csv");
          await utils.writeResults(
            failedResults,
            failedPath,
            constants.failedResultsHeader
  ...
  try {
    ...
    assetlist.res.forEach((file) => {
      if (
        ...
        console.log(
          "I will not send since the file is already processed or there are files being processed"
        );
      }
    });
